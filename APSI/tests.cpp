...
void testFullProtocol(uint32_t numParties, bool isEncrypted) {
    uint32_t numItem = 1<<20;
    uint32_t itemLen = 5;
    uint32_t prime = (1<<16) + 1;
    uint32_t remDepth = std::ceil(std::log2(numParties));
    HE bfv("BFV", 65537, isEncrypted + remDepth, numParties);    
...
void testFullProtocolTwoParty(int numParties) {
    ...
    std::vector<int64_t> queryMsg = msgVec[0];
    // std::cout << "Top 5 Items in Sender's Set: " << std::endl;
    // for (uint32_t i = 0; i < 5; i++) {
    //     std::cout << msgVec[i] << std::endl;
    // }
    // std::cout << "------------------------------" << std::endl;
    // std::cout << "Query: " << queryMsg << std::endl;
    ...
    if (!get<0>(retRes)) {
        ...
        // std::vector<int64_t> retMsg = bfv.decrypt(retCtxts[0])->GetPackedValue();
        // std::cout << std::vector<int64_t>(retMsg.begin(), retMsg.end()) << std::endl;
