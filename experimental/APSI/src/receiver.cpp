...
size_t ctxt_size(const OFHECtxt& ctxt) {
  size_t size = 0;
  for (const auto& element : ctxt->GetElements()) {
    for (const auto& subelements : element.GetAllElements()) {
...
int main(int argc, char **argv) {
  ...
  if (query_mode) {
    ...
    vector<std::pair<unsigned int, Ciphertext<DCRTPoly>>> ctexts =
      ...
      int i = 1;
      size_t total_ctxt_size = 0;
      for (const auto &x : ctexts) {
        total_ctxt_size += ctxt_size(x.second);
        i++;
      }

      std::cerr<< "Receiver's " << i << " receiver_windowed_vals ciphertexts size " << total_ctxt_size << std::endl;
      ...
      if (partialCiphertextVec.size() == 1) {
        ...
        res = finite_field_result(partialCiphertextVec[0], cryptoContext,
        ...
        res = finite_field_result(pt, cryptoContext, slots_used);
      }
